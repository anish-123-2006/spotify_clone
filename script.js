...
let sidebar = document.querySelector(".sidebar");
    ...
    for (let td of tds) {
        ...
        if (link && link.href.endsWith(".mp3")) {
            let text = link.innerText.replace(".mp3", "");
            let parts = text.split("-");
            let songname = parts[0];
            let artist = parts[1] || "Unknown Artist";

           songs.push({ url: link.href, name: songname });
            ...
            card.innerHTML = `
                <svg viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <text x="50%" y="150" text-anchor="middle" font-family="Arial, sans-serif" font-size="24" fill="black">
                    <tspan x="50%" dy="0">${songname}</tspan>
                  </text>
                </svg>
                ...
                <h3 class="transparent text">${songname}</h3>
                <p class="transparent description">${artist}</p>`;
            ...
            card.addEventListener("click", () => {
               currentIndex = songIndex;
                ...
                songdisplay.innerHTML=songname;
                                });
    ...
    player.addEventListener("timeupdate",()=>{
        ...
        let tt=(player.duration/60).toFixed(2);
        tt=tt.toString();
        tt=tt.replace(".",":");
total.innerHTML=tt;
